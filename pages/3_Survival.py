...
from dashboard.theme import ACCENT_RISK, apply_page_style, page_header, style_fig

apply_page_style()
if not require_api():
    st.stop()

page_header("M6 Survival Center", "Time-to-degradation curves, hazard ratios, and per-lot shelf-life signals.")
...
for col, (title, curves) in zip(cols, panels):
    ...
    col.plotly_chart(style_fig(fig).update_layout(title=title, yaxis_title="P(CT >= 60)", xaxis_title="Seasons"), width="stretch")
...
if not hazard.empty:
    ...
    c1.plotly_chart(style_fig(fig).update_layout(title="Cox PH Hazard Ratios", xaxis_title="log(HR)"), width="stretch")
...
st.plotly_chart(style_fig(fig).update_layout(title="Representative Individual Lot Curves", yaxis_title="P(CT >= 60)", xaxis_title="Seasons"), width="stretch")
